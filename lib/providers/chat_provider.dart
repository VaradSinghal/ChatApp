...
class ChatProvider extends ChangeNotifier {
  ...
  void loadChats(String currentUser) async {
    ...
    _debounce = Timer(Duration(seconds: 1), () async {
      Map<String, List<ChatDataModel>>? loadedChats = await currentUserChats(
        currentUser,
      );
  ...
  void addMessage(
    ...
    try {
      if (message.sender == currentUser) {
        ...
        _chats[message.receiver]!.add(
          ChatDataModel(message: message, users: users),
        );
      } else {
  ...
  void deleteMessage(MessageModel message, String currentUser) async {
    try {
      ...
      if (message.sender == currentUser) {
        _chats[message.receiver]!.removeWhere(
        ...
        _chats[message.sender]!.removeWhere(
          (element) => element.message == message,
        );
        deleteCurrentUserChat(chatId: message.messageId!);
